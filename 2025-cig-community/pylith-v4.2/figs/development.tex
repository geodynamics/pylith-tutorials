...
\newcommand{\level}[3]{%
  \node [anchor=west] at ($(#1.east)+(0,1.2ex)$) {\includegraphics[height=1.2em]{#2}};
...
\begin{tikzpicture}[
  ...
  \node (side-sets) [sustainability, below=of maintenance] {Face group BCs};\level{side-sets}{intermediate}{3, 100\%}
  \node (vtk)  [sustainability, below=of side-sets] {VTK $\rightarrow$ VTU};\level{vtk}{easy}{2, 100\%}
  ...
  \node (pybind11) [sustainability, below=of rate-rheologies] {SWIG $\rightarrow$ pybind11};\level{pybind11}{difficult}{5, 30\%}
...
% Performance
  ...
  \node (parallel-loading) [performance, below=of performance] {Parallel mesh loading};\level{parallel-loading}{expert}{5,90\%}
  \node (adaptive-timestepping) [performance, below=of parallel-loading] {Adaptive time stepping};\level{adaptive-timestepping}{easy}{1,80\%}
  \node (nondimensionalization) [performance, below=of adaptive-timestepping] {Improved nondimen.};\level{nondimensionalization}{intermediate}{2,25\%}
  \node (dmproject) [performance, below=of nondimensionalization] {DMProject optimization};\level{dmproject}{difficult}{3}
...
% Viscoelastic bulk rheologies for higher order TS (requires Disp+Vel)
  ...
  \node (point-sources) [features, below=of catmip-integration] {Point sources};\level{point-sources}{difficult}{4,85\%}
  ...
  \node (poro-visco) [features, below=of multiphysics] {Viscoelastic rheologies for poroelasticity};\level{poro-visco}{intermediate}{4, 45\%}
  \node (poro-faults) [features, below=of poro-visco] {Faults with poroelastic properties};\level{poro-faults}{difficult}{4, 80\%}
