...
\begin{frame}[t]
  ...
  \item Tag entities\\
    ...
    \begin{onlyenv}<2>
      ...
      \item Tag curves (2D) or surfaces (3D) for each boundary condition or fault
...
\begin{frame}
  ...
  \item Create fault traces
    ...
    \end{enumerate}\pause
  \item Embed fault surfaces (dim=2) in domain volume (dim=3) using {\ttfamily fragment}\pause
...
\begin{frame}
  ...
  \item Use helper function (included with PyLith) to improve mesh quality
...
\begin{frame}
  ...
  \hitem{Performance}
    ...
    \item Use basis order of 2 to get better accuracy with larger cells
    \end{itemize}
  \end{itemize}
...
% ------------------------------------------------------------ SLIDE
\begin{frame}
  \frametitle{{\ttfamily examples/subduction-3d} updated in v5.0.0}
  \summary{}

\begin{itemize}
\item Gmsh as default mesh generator
\begin{itemize}
  \item Mesh is not include in repository or binary package
  \item Refer to {\ttfamily input/README.md} for instructions on downloading mesh
  \item Demonstrates use of ETopo2020 for topography and bathymetry
  \end{itemize}
\item Cubit mesh
  \begin{itemize}
  \item Journal files $\rightarrow$ Python script
  \end{itemize}
\end{itemize}
